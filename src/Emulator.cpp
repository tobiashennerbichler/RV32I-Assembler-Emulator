...
void Emulator::updateScreen()
{
  ...
  y += ((int) ((end - start) / 16) + 2) * (NUMBER_HEIGHT + 2);
...
void Emulator::drawMemorySection(int x, int y, u32 start_address, u32 end_address)
{
  ...
  assert((y + ((size / 16) + 1) * (NUMBER_HEIGHT + 2)) < HEIGHT && "y out of range");
  assert((x + 20 * (NUMBER_WIDTH + 1) + 46 * NUMBER_WIDTH) < WIDTH && "x out of range");
